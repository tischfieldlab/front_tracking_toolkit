...
% Updated 26 December 2013 to use VideoReader instead of
% read_uncompressed_avi.m, and to be compatible with new version of
% unique.m. 
% Updated 16 August 2014 for compatibility with Norpix .seq movies (using
% Norpix2MATLAB.m). 
% Updated 4 June 2015 to use read_seq.m instead of Norpix2MATLAB.m, thereby
% achieving compatibility with newer .seq files. 
% Updated 27 July 2015 to fix bug saving movies with single-file input. 
% Updated 24 January 2017 to fix bug saving movies in subdirectory. 
% Updated 31 October 2017 to properly handle frames with no particles.
...
pausetime=1/10; % seconds to pause between frames when plotting
...
[x,y,t,ang,allt]=ParticleFinder(inputnames,threshold,framerange,[], ...
    bground_name,minarea,invert,0); % allt: list of all times
Nf=numel(allt);
if numel(x)==0
    error(['Sorry, found no particles in ' inputnames '.'])
end
if verLessThan('matlab','8')
    [tt,ends]=unique(t); % tt: all times with particles
else
    [tt,ends]=unique(t,'legacy');
end
...
% -=- Loop over frames -=-------------------------------------------------
for t=1:Nf
    indt = tt==allt(t); % necessary in case allt(1)~=1
    ind = begins(indt):ends(indt);
    Np = numel(ind); % particle count, this frame
    if Np>0
        if ~exist('tracks','var') % initialize tracks struct if needed
            active = 1:Np;
            n_active = numel(active);
            if minarea==1
                tracks = repmat(struct('len',[],'X',[],'Y',[],'T',[]), ...
                    Np,1);
                for ii = 1:Np
                    tracks(ii) = struct('len',1,'X',x(ind(ii)), ...
                        'Y',y(ind(ii)),'T',allt(t));
                end
            else
                tracks = repmat(struct('len',[],'X',[],'Y',[],'T',[], ...
                ...
                for ii = 1:Np
                    tracks(ii) = struct('len',1,'X',x(ind(ii)), ...
                        'Y',y(ind(ii)),'T',allt(t),'Theta',ang(ind(ii)));
                end
            end
            continue
        end % if ~exist('tracks','var')
        fr1=[x(ind) y(ind)];
        if any(minarea~=1)
            ang1=ang(ind);
        end
...
% -=- Match the tracks with kinematic predictions -=----------------------
        ...
        for ii = 1:n_active
            ...
            ind = links == bestmatch;
            if sum(ind) ~= 0
                if costs(ind) > costs(ii) % this match is better
                    links(ind) = 0;
                else % this match is not better
                    continue;
                end
            end
            links(ii) = bestmatch;
        end % for ii = 1:n_active
        ...
        for ii = 1:n_active
            if links(ii) ~= 0 
                ...
                tracks(active(ii)).T(end+1) = allt(t);
                if any(minarea~=1)
    ...
    disp(['Frame ' num2str(t) ' of ' num2str(Nf) ': ' ...
        num2str(Np,'%.0f') ' particles, ' ...
        num2str(n_active,'%.0f') ' active, ' ...
...
% -=- Plot if requested -=------------------------------------------------
if noisy
    [filepath,~,ext]=fileparts(inputnames);
    if isnumeric(noisy) && noisy>1
        disp('Plotting and saving frames. ')
        if exist(fullfile(filepath,savedirname),'file')~=7
            mkdir(fullfile(filepath,savedirname))
        end
    else
        disp('Plotting...')
    end
    defaultpos=get(0,'DefaultFigurePosition');
    hf=figure('units','pixels','position',[defaultpos(1:2) figsize]);
    ...
    if numel(names)==0
        error(['Sorry, cannot find ' inputnames '.']);
    end
    if ( strcmpi(ext,'.avi') || ...
    ...
    for ii=1:Nf-fitwidth
        ...
        if ( strcmpi(ext,'.avi') || ...
            ...
            strcmpi(ext,'.wmv') || strcmpi(ext,'.mpg') || strcmpi(ext,'.asf') || ...
            strcmpi(ext,'.asx') || strcmpi(ext,'.m4v') || strcmpi(ext,'.ogg') ) % single movie file
            im=readFrame(vobj,ii);
            set(hi,'cdata',im);
            set(hf,'name',[num2str(np) ' particles in ' names.name ...
                ' (' num2str(ii) ' of ' num2str(Nf) ')']); 
        elseif strcmpi(ext,'.seq')
            im=read_seq(inputnames,ii,0);
            set(hi,'cdata',im);
            set(hf,'name',[num2str(np) ' particles in ' names.name ...
                ' (' num2str(ii) ' of ' num2str(Nf) ')']); 
        elseif numel(names)==1 && ( strcmpi(ext,'.tif') || ...
            ...
            set(hf,'name',[num2str(np) ' particles in ' names(ii).name ...
                ' (' num2str(ii) ' of ' num2str(Nf) ')']); 
        end % if strcmpi(ext,'.avi') || ...
        drawnow
        pause(pausetime); 
        if isnumeric(noisy) && noisy>1
            ...
            if numel(names)==1
                imwrite(snap.cdata, ...
                    fullfile(filepath,savedirname, ...
                        [num2str(ii,'%05.0f') '.png'])); 
            else
                imwrite(snap.cdata, ...
                    fullfile(filepath,savedirname,names(ii).name)); 
            end
        end % if isnumeric(noisy) && noisy>1
    end % for ii=1:Nf-fitwidth
end % if noisy
disp('Done.')
